...
def get_authenticated_service(CLIENT_SECRETS_FILE):
  ...
  # credentials_path = os.path.join(home, "youtube-upload-credentials.json")
  credentials_path = "/content/youtube-upload-credentials.json"
  storage = Storage(credentials_path)
  credentials = storage.get()
# scope=["https://www.googleapis.com/auth/youtube.upload"])
  if credentials is None or credentials.invalid:
    print("NO FILEEEEEEEEEEEEEE LSFJLDKJFKLDJ")
    flow = flow_from_clientsecrets(CLIENT_SECRETS_FILE, SCOPES)
    #args = tools.argparser.parse_args(['--noauth_local_webserver'])
    # flow = InstalledAppFlow.from_client_secrets_file(CLIENT_SECRETS_FILE, SCOPES, storage)
    flags = tools.argparser.parse_args(args=['--noauth_local_webserver'])
    credentials = tools.run_flow(flow, storage, flags)
    #http = credentials.authorize(httplib2.Http())
    #flow = InstalledAppFlow.from_client_secrets_file(CLIENT_SECRETS_FILE, SCOPES, storage)
    # credentials = flow.run_console(storage)
